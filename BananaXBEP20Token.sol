 ...
 *  expectation is to have this happen in the _beforeTokenTransfer function of BEP20.
...
interface ITWAPOracle {
  ...
  function pcsCompPairAddressForLastEpochUpdateBlockTimstamp( address ) external returns ( uint32 );
  ...
  function updateTWAP( address pcsCompatPairAddressToUpdate_, uint eopchPeriodToUpdate_ ) external returns ( bool );
}
...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
  ...
  function addTWAPSource( address newTWAPSourceDexPool_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.add( newTWAPSourceDexPool_ ), "BananaxBEP20Token: TWAP Source already stored." );
    emit TWAPSourceAdded( newTWAPSourceDexPool_ );
  }

  function removeTWAPSource( address twapSourceToRemove_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.remove( twapSourceToRemove_ ), "BananaxBEP20Token: TWAP source not present." );
...
contract BananaXBEP20Token is Divine {

  using SafeMath for uint256;

    constructor() Divine("BananaX", "BANX", 9) {
    }
